...
def rotate2d(theta, **kwargs):
    return matrix([torch.cos(theta), torch.sin(-theta), 0], [torch.sin(theta), torch.cos(theta), 0], [0, 0, 1],
                  **kwargs)
...
def rotate3d(v, theta, **kwargs):
    ...
    return matrix([vx * vx * cc + c, vx * vy * cc - vz * s, vx * vz * cc + vy * s, 0],
                  ...
                  [vz * vx * cc - vy * s, vz * vy * cc + vx * s, vz * vz * cc + c, 0], [0, 0, 0, 1], **kwargs)
...
class AugmentPipe(torch.nn.Module):
    ...
    def forward(self, images, debug_percentile=None):
        ...
        v = np.asarray([1, 1, 1, 0]) / np.sqrt(3)  # Luma axis.
        if self.lumaflip > 0:
        ...
        if self.cutout > 0:
            ...
            size = torch.where(
                torch.rand([batch_size, 1, 1, 1, 1], device=device) < self.cutout * self.p, size,
